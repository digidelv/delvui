/**
 * DelvUI Vanilla JS Icon Component
 * Scalable icon Web Component with built-in icon library
 */

import { AtomComponent } from '@delvui/core';
import './Icon.css';

export class Icon extends AtomComponent {
  static tagName = 'delv-icon';
  ...
  constructor() {
    ...
    // Built-in icon library (same as other frameworks)
    this.ICONS = {
      // UI Icons
  ...
  render() {
    ...
    this.setAttribute('data-delvui-component', 'icon');
  ...
  getIconSize() {
    if (!isNaN(this.size)) return parseInt(this.size);
